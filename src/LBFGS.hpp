...
// From: https://github.com/mattoverby/mclgeom
...
// L-BFGS implementation based on Nocedal & Wright Numerical Optimization book
// (Section 7.2) adapted from code by Ioannis Karamouzas.
//
...
class LBFGS {
   public:
    ...
    struct Options {
        ...
        Options() : min_iters(0), max_iters(30), abs_tol(1e-3), rel_tol(0), M(6), gamma(1) {}
    } options;
    ...
    // Optional:
    // Returns true if the solver should exit, default uses ||g||<abs_tol or
    // ||g||<||x||rel_tol
...
template <typename MatrixType>
LBFGS<MatrixType>::LBFGS() : initialized(false), num_iters(0), max_iters(0), k(0), gamma_k(1), obj_0(0), obj_k(0) {}
...
typename LBFGS<MatrixType>::Scalar LBFGS<MatrixType>::iterate(MatrixType& velocity) {
    ...
    //
    {
        ...
        descent = -q;
        x_last = velocity;
        obj_k = linesearch(velocity, grad, descent, step_size);
        if (num_iters >= options.min_iters && converged(obj_k, x_last, velocity, grad)) {
            return obj_k;
        }
    }
...
template <typename MatrixType>
typename LBFGS<MatrixType>::Scalar LBFGS<MatrixType>::bracketing_weakwolfe(MatrixType& velocity,
                                                                           MatrixType& g,
    ...
    int x_cols = velocity.cols();

    g = MatrixType::Zero(velocity.rows(), x_cols);
    MatrixType xp = velocity;
    const Scalar fx_init = gradient(velocity, g);
    Scalar fx = fx_init;
    ...
    for (iter = 0; iter < maxiter; iter++) {
        velocity = xp + step * drt;
        fx = gradient(velocity, g);
...
template <typename MatrixType>
bool LBFGS<MatrixType>::default_converged(Scalar curr_obj,
...
template <typename MatrixType>
typename LBFGS<MatrixType>::Scalar LBFGS<MatrixType>::inner(const MatrixType& a, const MatrixType& b) const {
